...
When there is a surface to preview, run the `$user-experience` skill by reading and following the installed skill instructions to load the UX lens, so what you point the user at and how you weigh their verdict stay anchored to the user's goal and the full path through the change.
