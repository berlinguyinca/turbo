...
3. **Load the UX lens for user-facing work.** If the task changes any user-facing surface — UI, screens, commands, messages, or any behavior a user sees or does — run the `/user-experience` skill via the Skill tool to load the UX lens, so escalated product decisions are weighed by their cost to the user's goal and flow. Skip for internal-only work with no user-facing surface.
