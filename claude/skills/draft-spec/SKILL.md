...
3. **Load the UX lens for user-facing work.** If the project has any user-facing surface — UI, screens, commands, messages, or any behavior a user sees or does — run the `/user-experience` skill via the Skill tool to load the UX lens, so the deep-dive stays anchored to the user's goal and examines the flow as requirements are gathered. Skip for internal-only work with no user-facing surface.
